...
Vector3d pre_T(0,0,0);
Vector3d pre_odom_msg_pose(0,0,0);
Quaterniond pre_R(0,0,0,0);
Quaterniond pre_odom_msg_ori(0,0,0,0);

double sum_of_trans_error=0;
double sum_of_rotation_error=0;
bool start_flag=false;
double num_of_frame=0;
void odom_callback(const nav_msgs::OdometryConstPtr &odom_msg)
{
    ...
    else
    {
      ...
      tmp_R.normalize();
      pre_R.normalize();
      Quaterniond diff_R=tmp_R*pre_R;
      Quaterniond diff_odom_ori=Quaterniond(odom_msg->pose.pose.orientation.w,
                                            odom_msg->pose.pose.orientation.x,
                                            odom_msg->pose.pose.orientation.y,
                                            odom_msg->pose.pose.orientation.z)*pre_odom_msg_ori;
      diff_R.normalize();
      Matrix3d diff_R_Euler=diff_R.toRotationMatrix();
      Matrix3d diff_odom_ori_Euler=diff_odom_ori.toRotationMatrix();
      Matrix3d diff_R_Euler_T=diff_R_Euler.transpose();
      //diff_odom_ori_Euler.normalize();
      //diff_R_Euler_T.normalize();
      Matrix3d Rgt_R=diff_R_Euler_T*diff_odom_ori_Euler;
      //Rgt_R.normalize();
      if(isnan(acos((Rgt_R.trace()-1)/2)))
      {
         std::cout<<"nan nan nan"<<std::endl;
         std::cout<<"Rgt_R trace : "<<Rgt_R.trace()<<std::endl;
         std::cout<<"Rgt_R determinant : "<<Rgt_R.determinant()<<std::endl;
         std::cout<<"diff_R_Euler determinant : "<<diff_R_Euler_T.determinant()<<std::endl;
         std::cout<<"diff_R_Euler_T determinant : "<<diff_R_Euler_T.determinant()<<std::endl;
      }
      else
      {
        num_of_frame+=1;
        sum_of_rotation_error+=acos((Rgt_R.trace()-1)/2);
        std::cout<<"rotation error hello : "<<sum_of_rotation_error/num_of_frame<<std::endl;

        //translation error
        Vector3d diff_T=tmp_T-pre_T;
        Vector3d diff_odom_pose=Vector3d{odom_msg->pose.pose.position.x,
            odom_msg->pose.pose.position.y,
            odom_msg->pose.pose.position.z}-pre_odom_msg_pose;
        Vector3d error_of_Trans=diff_T-diff_odom_pose;
        sum_of_trans_error+=error_of_Trans.dot(error_of_Trans);
        std::cout<<"translation error : "<<sqrt(sum_of_trans_error/num_of_frame)<<std::endl;
      }
    }

    pre_T=tmp_T;
    pre_odom_msg_pose=Vector3d{odom_msg->pose.pose.position.x,
                               odom_msg->pose.pose.position.y,
                               odom_msg->pose.pose.position.z};

    pre_R=tmp_R;
    pre_odom_msg_ori=Quaterniond(odom_msg->pose.pose.orientation.w,
                                 odom_msg->pose.pose.orientation.x,
                                 odom_msg->pose.pose.orientation.y,
                                 odom_msg->pose.pose.orientation.z);
