    ...
    while (true)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                ...
                double stereo_depth=img_msg->channels[5].values[i];
                ROS_ASSERT(z == 1);
                Eigen::Matrix<double, 8, 1> xyz_uv_velocity_depth;
                xyz_uv_velocity_depth << x, y, z, p_u, p_v, velocity_x, velocity_y,stereo_depth;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_relo_points = n.subscribe("/pose_graph_stereo/match_points", 2000, relocalization_callback);
